...
/**
 * 🎨 CanvasStage — The Main Editor Surface
 * 
 * Orchestrates the camera, scaling, selection overlays, and slide rendering.
 */
export const CanvasStage = memo(function CanvasStage() {
  const stageRef = useRef<HTMLDivElement>(null)
  const { isReadOnly, isAuthenticated } = useAccessControl()

  // 1. Hook-based Logic (Audit: useCanvasCamera uses useLayoutEffect for performance)
  ...
  // 2. Scale Calculation (Audit: ResizeObserver is industry standard)
  ...
  // 3. Selection Logic (Audit: useMemo prevents recalculation on unrelated store changes)
